...
@hydra.main(
		...
		config_name="exp_hg_kitti_ts.yaml"
)
def main(cfg: DictConfig):
	...
	# load data
	camera_K, seq_len = process_data(cfg.data_root, cfg.seq_num)
	...
	# load ground truth
	gt_dir = os.path.join(cfg.repo_root, "results", "gt_kitti")
	...
	for i in range(seq_len - 1):
		img1 = kitti_funcs.read_kitti_img_file(cfg.data_root, cfg.seq_num, i, invert_channels=True)
		img2 = kitti_funcs.read_kitti_img_file(cfg.data_root, cfg.seq_num, i+1, invert_channels=True)
		...
		img1 = img_funcs.proc_for_loftr(img1)
		img2 = img_funcs.proc_for_loftr(img2)
		...
		pitch_gt = hg_funcs.calculate_pitch_from_normal(normal_gt, frame="x-forward")
		pitch_refined = hg_funcs.calculate_pitch_from_normal(normal_refined, frame="x-forward")
