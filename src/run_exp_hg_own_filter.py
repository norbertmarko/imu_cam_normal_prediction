...
@hydra.main(
		...
		config_name="exp_hg_own_ts.yaml"
)
def main(cfg: DictConfig):
	...
	# load data
	camera_K, seq_len = process_data(cfg.processed_data_root, cfg.seq_num)
	...
	if cfg.roi_img_dst_size.own is not None:
		roi_img_dst_size = cfg.roi_img_dst_size.own
	else:
		roi_img_dst = own_funcs.read_img_file(
			cfg.processed_data_root, cfg.seq_num, frame_num=0
		).shape[:2]
		roi_img_dst_size = (roi_img_dst[1], roi_img_dst[0])

	roi = hg_funcs.calculate_scaled_roi(roi, roi_img_src_size, roi_img_dst_size)

	# rescale camera intrinsics (if needed)
	if cfg.roi_img_dst_size.own is not None:
		camera_K = hg_funcs.calculate_scaled_intrinsics(
			camera_K, roi_img_src_size, roi_img_dst_size)
	

	# load ground truth
	gt_dir = os.path.join(cfg.repo_root, "results", "gt_own")
	gt_path = os.path.join(gt_dir, cfg.seq_num, "gt", f"gt_data_{cfg.seq_num}.pkl")
	...
	# plane change analysis variables
	prev_normal, prev_rotation = None, None
	...
	# iterate over consecutive image pairs (too many images, load on the fly)
	for i in range(seq_len - 1):
		K = 1
		img1 = own_funcs.read_img_file(cfg.processed_data_root, cfg.seq_num, i, invert_channels=False)
		img2 = own_funcs.read_img_file(cfg.processed_data_root, cfg.seq_num, i+K, invert_channels=False)
		...
		pitch_gt = hg_funcs.calculate_pitch_from_normal(normal_gt, frame="x-backward")
		pitch_refined = hg_funcs.calculate_pitch_from_normal(normal_refined, frame="x-backward")
		...
		if cfg.quicktest:
			# quick test - break after x frames
			if i == 149:
				break
